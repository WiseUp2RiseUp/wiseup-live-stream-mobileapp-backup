...
public class LocalParticipant extends Participant {
    ...
    public void startCamera(boolean v, boolean c) {
        ...
        if (!v){
         ...
         VideoCapturer videoCapturer = createCameraCapturer(c);
        ...
        {
         UvcCapturer usbCapturer = new UvcCapturer(context, localVideoView);
         ...
         usbCapturer.startCapture(1920, 1080, 60);
         }
    ...
    private VideoCapturer createCameraCapturer(Boolean v) {
        ...
        if(v) return backCam(enumerator);
        else return frontCam(enumerator);
    }

    private VideoCapturer backCam(CameraEnumerator enumerator){
    ...
    private VideoCapturer frontCam(CameraEnumerator enumerator){
        final String[] deviceNames = enumerator.getDeviceNames();
        for (String deviceName : deviceNames) {
            if (enumerator.isFrontFacing(deviceName)) {
                videoCapturer = enumerator.createCapturer(deviceName, null);
